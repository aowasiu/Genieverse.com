<?php
	...
	{
		global $db;
		$query = 'SELECT mall_post_id FROM gv_mall_post WHERE (mall_post_block = 0) AND (mall_post_id = ' . $data1 . ' AND member_id = ' . $data2 . ')';
	...
	{
		global $db;
		$query = 'SELECT mall_post_id FROM gv_mall_post_image WHERE (mall_image_block = 0) AND mall_post_id = ' . $data;
		$result = mysql_query($query, $db) or die(mysql_error($db));
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] == "Upload photo")
	{
		...
		if(count($errors) < 1)
		{
			...
			$_SESSION["errand"] = '<br><a href="mall/member/new_post_photo.php?id=' . $post_id .'">Click here to upload more photos.</a>' . '<br> or <a href="mall/member/my_post.php?id=' . $post_id .'">Click here to continue.</a>';
