<?php
	...
	{
		global $db;
		$query = 'SELECT mall_image_id FROM gv_mall_post_image WHERE (mall_image_block = 0 AND mall_post_id = ' . mysql_real_escape_string($data1, $db) . ') AND mall_image_filename = "' . mysql_real_escape_string($data2, $db) . '"';
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] == "Change photo")
	{
		...
		if(count($errors) < 1)
		{
			...
			$query = 'DELETE FROM gv_mall_post_image WHERE mall_image_filename = "' . mysql_real_escape_string($post_name, $db) . '"';
			...
			$query = 'INSERT INTO gv_mall_post_image
						(mall_image_id, mall_post_id, mall_image_block, mall_image_assessment, mall_image_assessed_by, mall_image_created_on, mall_image_edited_on, mall_image_caption, mall_image_filename)
						VALUES
			...
			$query = 'UPDATE gv_mall_post_image SET mall_image_filename="' . $image_name . '" WHERE mall_image_id = ' . $last_id;
			mysql_query($query, $db) or die(mysql_error($db));
			...
			$_SESSION["errand"] = '<br><a href="mall/member/new_post_photo.php?id=' . $post_id . '&name=' .$post_name . '">Click here to upload more photos.</a>' . '<br> or <a href="mall/member/my_post.php?id=' . $post_id . '">Click here to continue.</a>';
	...
	$extra_title = "New post photo | Genieverse Mall - Free web market ";
	$shop_item = "set";
    ...
    <div class="container">
		...
		<div class="row">
			<div><!--class="col-md-9"-->	
				<div class="row">
					<div class="col-md-12">
						<span class="pull-right btn btn-info"><a href="my_post.php?id=<?php echo $get_id; ?>">Back to post</a></span>
