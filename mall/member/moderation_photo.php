<?php
	...
	if($action !== "" && $get_id !== "")
	{
		if($action == 'approve')
		{
			$query = 'UPDATE gv_mall_post_image SET mall_image_assessment=1, mall_image_assessed_by="' . $_SESSION["username"] . '", mall_image_assessed_on=NOW() WHERE mall_image_block = 0 AND mall_image_filename = ' . $get_id;
		}
		elseif($action == 'disapprove')
		{
			$query = 'UPDATE gv_mall_post_image SET mall_image_assessment=1, mall_image_assessed_by="' . $_SESSION["username"] . '", mall_image_assessed_on=NOW(), mall_image_block=1 WHERE mall_image_block = 0 AND mall_image_filename = ' . $get_id;
		}
		else{
			//Do nought
		}
	...
	$query = 'SELECT mall_image_id, mall_post_id, mall_image_created_on, mall_image_edited_on, mall_image_caption, mall_image_filename	FROM	gv_mall_post_image	WHERE	(mall_image_block = 0 AND mall_image_assessment = 0) AND (mall_image_assessed_by = "None") LIMIT 1';
	...
	if($result = mysql_query($query))
	{
		if(mysql_num_rows($result) < 1)
		{
			...
			exit;
				...
				$photos_id = $row["mall_image_id"];
				$photos_creation = $row["mall_image_created_on"];
				$photos_editing = $row["mall_image_edited_on"];
				$photos_caption = $row["mall_image_caption"];
				$photos_filename = $row["mall_image_filename"];
		}//End of if(mysql_num_rows($result) < 1)
	}//End of if($result = mysql_query($query))
	
	$posts_category_id = mall_category_id($posts_id);
	
	$extra_title = "Photo Moderation | Genieverse Mall - Free web market ";
	$shop_item = "set";
	...
	<a href="../search.php?category=appliances" class="list-group-item<?php if($the_category_name == "appliances"){ echo ' active'; }?>">Appliances</a><?php
		...
		getCategoryList();
            ...
            <div class="col-md-9">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <span class="pull-right"><?php echo mall_category_name($posts_category_id); ?></span>
                            <p><b>Category</b></p>
                        </div>
                    </div>
					...
					<div class="row">
                        <div class="col-md-12">
                            ...
                            <img class="img-responsive" src="../../images/service_images/mall/<?php echo $photos_filename; ?>"  alt="<?php
						  if($photos_caption == "")
						  {	echo "No caption.";	}
...
<?php require_once('../../Server_Includes/scripts/common_scripts/member_page_common_before_body_end.php'); ?>
	...
	$query = 'UPDATE gv_mall_post_image SET mall_image_assessed_by = "moderator" WHERE mall_image_id = ' . $photos_id;
	mysql_query($query, $db) or die(mysql_error($db));
