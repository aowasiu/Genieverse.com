<?php
	...
	$query = 'SELECT mall_post_id, mall_category_id, cc_id, mall_post_created_on, mall_post_title, mall_post_description, mall_post_state, mall_post_city FROM gv_mall_post WHERE (mall_member = 1 AND mall_post_block = 0) AND (member_id = ' . $get_id . ')ORDER BY mall_post_id DESC ';
	...
	$extra_title = member_username($get_id) . "'s posts | Genieverse Mall - Free web market. ";	
...
?>
		...
		echo modified_for_url($row["mall_post_title"], "&Post=");
		?>"><img class="img-responsive" height="65" width="65" src="../images/service_images/mall/<?php echo post_image($row["mall_post_id"]); ?>"></a>
                <h4><a href="post_view.php?id=<?php
		echo  $row["mall_post_id"];
		echo modified_for_url(category_name($row["mall_category_id"]), "&Category=");
		echo modified_for_url($row["mall_post_title"], "&post="); ?>"><?php echo $row["mall_post_title"]; ?></a></h4>
                <p><?php echo truncate_text(html_entity_decode($row["mall_post_description"]), 40, "No detail."); ?></p>
            </div>
<?php
		}
	}
			...
			?>
            <div class="col-md-4 portfolio-item">
                <p>Sorry. There're no recent Mall posts.</p>
			</div><?php
	}?>
        </div>
		<!-- /.row -->
