<?php
	...
	//	$country_id = (isset($_POST["country_id"]))	?	$_POST["country_id"] : 72;
		...
		if(count($errors) < 1)
		{
			...
			$query = 'UPDATE gv_spotlight_post SET
					  spotlight_post_investigated_on=NOW(),
					  spotlight_post_edited_on=NOW(),
					  spotlight_post_assessment=0,
					  spotlight_post_assessed_by="None",
					  spotlight_category_id=' . mysql_real_escape_string($category_id, $db) . ',
					  cc_id=' . mysql_real_escape_string(country_id($_SESSION["member_id"]), $db) . ', ' . 'spotlight_post_youtube="' . mysql_real_escape_string(filter_var($safe_youtube, FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_HIGH), $db) . '",
					  spotlight_post_title="' . mysql_real_escape_string(filter_var($safe_title, FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_HIGH), $db) . '",
					  spotlight_post_body="' . mysql_real_escape_string($safe_description, $db) . '", spotlight_post_state="' . mysql_real_escape_string(filter_var($correct_state, FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_HIGH), $db) . '",
					  spotlight_post_city="' . mysql_real_escape_string(filter_var($correct_city, FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_HIGH), $db) . '", spotlight_post_contact_name="' . mysql_real_escape_string($correct_contact_name, $db) . '", spotlight_post_contact_phone="' . mysql_real_escape_string($safe_contact_phone, $db) . '", spotlight_post_contact_email="' . mysql_real_escape_string($safe_contact_email, $db) . '", spotlight_post_contact_website="' . mysql_real_escape_string($contact_website, $db) . '"
					  WHERE 
					  spotlight_post_id = ' . mysql_real_escape_string($post_id, $db) . '
	...
	$query = 'SELECT spotlight_category_id, cc_id, spotlight_post_youtube, spotlight_post_title, spotlight_post_body, spotlight_post_state, spotlight_post_city, spotlight_post_contact_name, spotlight_post_contact_phone, spotlight_post_contact_email,  spotlight_post_contact_website FROM gv_spotlight_post WHERE  (member_id = ' . $_SESSION["member_id"] . ' AND spotlight_post_id = ' . mysql_real_escape_string($post_id, $db) . ') AND spotlight_post_block = 0';
	$result = mysql_query($query, $db) or die (mysql_error($db));
	...
	if($result = mysql_query($query))
	{
		...
		if(mysql_num_rows($result) > 0)
		{
			...
			$category_id =  $row["spotlight_category_id"];
			$title = $row["spotlight_post_title"];
			$description =  $row["spotlight_post_body"];
			$country_id =  $row["cc_id"];
			$youtube =  $row["spotlight_post_youtube"];
			$state =  $row["spotlight_post_state"];
			$city =  $row["spotlight_post_city"];
			$contact_name =  $row["spotlight_post_contact_name"];
			$contact_phone =  $row["spotlight_post_contact_phone"];
			$contact_email =  $row["spotlight_post_contact_email"];
			$contact_website =  $row["spotlight_post_contact_website"];
	...
	$extra_title = "Post editing | Genieverse Spotlight - " . $spotlight_title_description;
	$shop_item = "set";
