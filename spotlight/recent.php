<?php
	...
	$query = 'SELECT spotlight_post_id, spotlight_category_id, cc_id, spotlight_post_created_on, spotlight_post_title, spotlight_post_body, spotlight_post_state, spotlight_post_city FROM gv_spotlight_post WHERE (spotlight_member = 1 AND spotlight_post_block = 0) ORDER BY spotlight_post_id DESC ';
	...
	$extra_title = "Recent posts | Genieverse Spotlight - " . $spotlight_meta_description;	
...
<?php require_once('../Server_Includes/scripts/spotlight_scripts/spotlight_outer_page_header.php'); ?>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><small>Recent posts</small></h1>
            </div>
        </div>
        <!-- /.row -->
<?php
...
?>
		...
		?>"><img class="img-responsive" height="65" width="65" src="../images/service_images/spotlight/<?php echo post_image($row["spotlight_post_id"]); ?>"></a>
...
<?php
			...
			?>
            <div class="col-md-4 portfolio-item">
                <p>Sorry. There're no recent Spotlight posts.</p>
			</div><?php
	}?>
        </div>
		<!-- /.row -->
