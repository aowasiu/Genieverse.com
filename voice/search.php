<?php
	...
	/*
		This script searches Voice table of posts to retrieve posts matching the 'search_keywords'.
		It also draws from Voice post image table, related photos or images
	*/
	...
	if(!isset($url_category_name) || $url_category_name == "")
	{
		...
		$_SESSION["errand"] = 'You need to select a category in the <a href="categories.php?service=voice">Categories</a> section to view posts.';
	...
	//Find out i category name exists in the database
	...
	if($category_id == 0)
	{
		...
		$_SESSION["errand"] = 'The category you requested is not available.<br/><a href="categories.php?service=voice">Click here to choose from available categories</a>';
		header("Location: ../issues.php");
		exit;
	}
	
	$query = 'SELECT voice_post_id, cc_id, voice_category_id, voice_post_settlement, voice_post_title, voice_post_body, voice_post_state, voice_post_city, voice_post_created_on FROM gv_voice_post WHERE (voice_member = 1 AND voice_post_block = 0) AND (voice_category_id = ' . $category_id . ') ORDER BY voice_post_id DESC ';
/*
	(voice_post_settlement = 1 AND voice_category_id = ' . $category_id . ') ORDER BY voice_post_id DESC ';
*/
	...
	if($rows == 0)
	{
		...
		$_SESSION["errand"] = "There're no posts, yet, for this category<br/><a href='categories.php?service=voice'>Click here to choose another category besides " . ucfirst($category_name) . '</a>';
		header("Location: ../issues.php");
		exit;
	}
	...
	//Set extra title
	$extra_title = "Search | Genieverse Voice - Tell the world what's happening.";
...
?>
		 ...
		 echo $row["voice_post_id"];
		 echo modified_for_url($row["voice_post_title"], "&Post="); ?>"><?php echo $row["voice_post_title"]; ?></a></h3>
