<?php
	...
	{
		global $db;
		$query = 'SELECT voice_post_id FROM gv_voice_post WHERE (voice_post_block = 0) AND (voice_post_id = ' . $data1 . ' AND member_id = ' . $data2 . ')';
	...
	{
		global $db;
		$query = 'SELECT voice_post_id FROM gv_voice_post_image WHERE (voice_image_block = 0) AND voice_post_id = ' . $data;
		$result = mysql_query($query, $db) or die(mysql_error($db));
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] == "Upload photo")
	{
		...
		if(count($errors) < 1)
		{
			...
			$_SESSION["errand"] = '<br><a href="voice/member/new_post_photo.php?id=' . $post_id .'">Click here to upload more photos.</a>' . '<br> or <a href="voice/member/my_post.php?id=' . $post_id .'">Click here to continue.</a>';
