<?php
	...
	{
		global $db;
		$query = 'SELECT voice_image_id FROM gv_voice_post_image WHERE (voice_image_block = 0 AND voice_post_id = ' . mysql_real_escape_string($data1, $db) . ') AND voice_image_filename = "' . mysql_real_escape_string($data2, $db) . '"';
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] == "Change photo")
	{
		...
		if(count($errors) < 1)
		{
			...
			$query = 'DELETE FROM gv_voice_post_image WHERE voice_image_filename = "' . mysql_real_escape_string($post_name, $db) . '"';
			...
			$query = 'INSERT INTO gv_voice_post_image
						(voice_image_id, voice_post_id, voice_image_block, voice_image_assessment, voice_image_assessed_by, voice_image_created_on, voice_image_edited_on, voice_image_caption, voice_image_filename)
						VALUES
			...
			$query = 'UPDATE gv_voice_post_image SET voice_image_filename="' . $image_name . '" WHERE voice_image_id = ' . $last_id;
			mysql_query($query, $db) or die(mysql_error($db));
			...
			$_SESSION["errand"] = '<br><a href="voice/member/new_post_photo.php?id=' . $post_id . '&name=' .$post_name . '">Click here to upload more photos.</a>' . '<br> or <a href="voice/member/my_post.php?id=' . $post_id . '">Click here to continue.</a>';
	...
	$extra_title = "New post photo | Genieverse Voice - Tell the world what's happening. ";
	$shop_item = "set";
