<?php
	...
	$special_button = 'Mark this matter as Resolved.';
	$delete_button = 'Delete all photos';
	$delete_button2 = 'Delete this post';
	$service_tagPlus = 'settle';
	$service_tagNegative = 'nonsettle';
	...
	$image_directory = $image_directory_core . '/images/service_images/voice/';
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["delete"] == "Delete this photo")
	{
		...
		$query = 'DELETE FROM gv_voice_post_image WHERE voice_image_filename = "' . mysql_real_escape_string($post_name, $db) . '" AND voice_post_id = ' . mysql_real_escape_string($post_id, $db);
	...
	{
		$query = 'SELECT voice_image_filename FROM gv_voice_post_image WHERE voice_post_id = ' . mysql_real_escape_string($post_id, $db);
		...
		while($row = mysql_fetch_assoc($result))
		{
			...
			if(mysql_num_rows($result) > 0)
			{
				...
				if(in_array($row["voice_image_filename"], $existing_images))
				{
					unlink($image_directory . '/' . $row["voice_image_filename"]);
				}
			}
		}

		$query = 'DELETE FROM gv_voice_post_image WHERE voice_post_id = ' . mysql_real_escape_string($post_id, $db);
		mysql_query($query, $db) or die (mysql_error($db));
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["delete"] == $delete_button2)
	{
		...
		$query = 'DELETE FROM gv_voice_post WHERE voice_post_id = ' . mysql_real_escape_string($post_id, $db) . ' AND member_id = ' . $_SESSION["member_id"];
		mysql_query($query, $db) or die (mysql_error($db));
	...
	{
		$query = 'UPDATE gv_voice_post SET voice_post_settlement=1, voice_post_settled_on=NOW() WHERE voice_post_id = ' . mysql_real_escape_string($post_id, $db);
		mysql_query($query, $db) or die(mysql_error($db));
		header("Location: my_post.php?id=$post_id");
		exit;
	}
	...
	//Get Voice meta data and footer
	...
	extract($row);
	$the_category_name = voice_category_name($row["voice_category_id"]);
	...
	$extra_title =  $row["voice_post_title"] . " | My posts | Genieverse Voice - Tell the world what's happening. ";
               ...
               <div class="thumbnail">
                ...
                <div class="thumbnail">
                    <div class="caption-full">
						...
						<?php if(!empty($row["voice_post_youtube"])){ ?><iframe class="col-sm-8 col-lg-12" title="YouTube video player" class="youtube-player" type="text/html" width="425" height="349" src="http://www.youtube.com/embed/<?php echo $row["voice_post_youtube"]; ?>"
						...
						<br>
						<hr><b>Location:</b><hr>
						<p><?php echo $row["voice_post_city"];
						if($row["voice_post_city"] !== ""){ echo ", "; }
						echo $row["voice_post_state"];	?></p>
						<br>
						<hr><b>Contact:</b><hr>
						...
						<br>
						<hr><b>Website:</b><hr>
						<p><?php echo $row["voice_post_contact_website"]; ?></p>
						<br>
						<hr><b>You created this post on:</b><hr>
						<p><?php	echo change_date_long($row["voice_post_created_on"]); ?></p>
						<br>
...
<?php
	if($row["voice_post_settlement"] == 0)
	{
?>
					<div class="caption-full">
...
?>					<div class="caption-full">
                        <h5 class="pull-right"></h5>
                        <h5><p class="alert alert-info">This matter has been resolved</p></h5>
						<hr>
					</div>
<?php
	}
...
?>
			<div class="col-lg-3 col-md-4 col-xs-6 thumb">
				<a href="../../images/service_images/voice/<?php echo $row["voice_image_filename"]; ?>" class="fancybox thumbnail" <?php if(mysql_num_rows($result) > 1){ echo ' rel="gallery"';}?> title="<?php
				if($row["voice_image_caption"] == ""){ echo "No caption"; }
				else{ echo $row["voice_image_caption"]; } ?>"><img <?php // class="img-responsive" ?> src="../../images/service_images/voice/<?php echo $row["voice_image_filename"]; ?>"  style="height:30%; width:30%;" alt="<?php echo $data2; ?>"></a>
				...
				<form class="text-center" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
					...
					<input type="hidden" name="name" value="<?php echo $row["voice_image_filename"]; ?>" />
					<input class="btn btn-default" type="submit" name="delete" value="Delete this photo" />
				</form>
				<p class="text-center"><a class="btn btn-default" href="update_post_photo.php?id=<?php echo $data; ?>&name=<?php echo $row["voice_image_filename"]; ?>">Change this photo</a></p>
			   <p><b>Created on:</b> <?php echo get_formated_date($row["voice_image_created_on"]); ?></p>
			   <p><b>Edited on:</b> <?php echo get_formated_date($row["voice_image_edited_on"]); ?></p>
			</div>
...
<?php
					...
					echo get_post_photos($get_id, $row["voice_post_title"], $post_id);
				?>
		</div>
