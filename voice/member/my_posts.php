<?php
	...
	if(isset($post_class) && $post_class == 'settled')
	{
		$post_activity = 1;
	}
	elseif(isset($post_class) && $post_class == 'nonsettled')
	{
		$post_activity = 0;
	}	
	else {
	...
	if(isset($delete) && $delete !== 0) 
	{
		...
		$query = 'SELECT voice_image_filename FROM gv_voice_post_image WHERE voice_post_id = ' . mysql_real_escape_string($delete, $db);
		...
		$query = 'DELETE FROM gv_voice_post_image WHERE voice_post_id = ' . mysql_real_escape_string($delete, $db);
		mysql_query($query, $db) or die (mysql_error($db));

		$query = 'DELETE FROM gv_voice_post WHERE voice_post_id = ' . mysql_real_escape_string($delete, $db) . ' AND member_id = ' . $_SESSION["member_id"];
		mysql_query($query, $db) or die (mysql_error($db));
	...
	//Get Genieverse Voice post's first image function
	...
	if($post_class !== "")
	{
		$query = 'SELECT voice_post_id, voice_post_settlement, voice_post_created_on, voice_post_title FROM gv_voice_post WHERE (voice_post_block = 0 AND voice_member = 1) AND (member_id = ' . $_SESSION["member_id"] . ' AND voice_post_settlement = ' . mysql_real_escape_string($post_activity, $db) . ') ORDER BY voice_post_created_on DESC ';
	}
	else {
			$query = 'SELECT voice_post_id, voice_post_settlement, voice_post_created_on, voice_post_title FROM gv_voice_post WHERE (voice_post_block = 0 AND voice_member = 1) AND member_id = ' . $_SESSION["member_id"] . ' ORDER BY voice_post_created_on DESC ';
	}
	...
	$extra_title = ucfirst($_SESSION["username"]) . " posts | Genieverse Voice - Tell the world what's happening. ";	
		...
		getCategoryList();
            ...
            <div class="col-md-9">
				...
				<div class="caption-full">
					<h3>My <?php if($post_class !== ""){ echo ucfirst($post_class); } ?> Voice posts</h3>
				</div>
				<br>
...
?>				<tr class="<?php echo $the_css; ?>caption-full">
					<td><a href="my_post.php?id=<?php echo $row["voice_post_id"]; ?>"><img class="img-responsive" height="60" width="60" src="../../images/service_images/voice/<?php echo post_image($row["voice_post_id"]); ?>" alt="Genieverse Voice - Tell the world what's happening."></a></td>
					<td><a href="my_post.php?id=<?php echo $row["voice_post_id"]; ?>"><?php echo truncate_text($row["voice_post_title"], 80, "No title"); ?></a></td>
					<td><?php echo custom_date($row["voice_post_created_on"], "F t, Y. g:ia"); ?></td>
					<td><a class="btn btn-default" href="my_posts.php?delete=<?php echo $row["voice_post_id"]; ?>">Delete</a></td>
				</tr>
<?php
	}//End of while loop
