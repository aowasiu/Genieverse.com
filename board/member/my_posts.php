<?php
	...
	if(isset($post_class) && $post_class == 'investigated')
	{
		$post_activity = 1;
	}
	elseif(isset($post_class) && $post_class == 'noninvestigated')
	{
		$post_activity = 0;
	}
	else {
	...
	if(isset($delete) && $delete !== 0) 
	{
		...
		$query = 'SELECT spotlight_image_filename FROM gv_spotlight_post_image WHERE spotlight_post_id = ' . mysql_real_escape_string($delete, $db);
		...
		$query = 'DELETE FROM gv_spotlight_post_image WHERE spotlight_post_id = ' . mysql_real_escape_string($delete, $db);
		mysql_query($query, $db) or die (mysql_error($db));

		$query = 'DELETE FROM gv_spotlight_post WHERE spotlight_post_id = ' . mysql_real_escape_string($delete, $db) . ' AND member_id = ' . $_SESSION["member_id"];
		mysql_query($query, $db) or die (mysql_error($db));
	...
	//Get Genieverse Spotlight post's first image function
	...
	if($post_class !== "")
	{
		$query = 'SELECT spotlight_post_id, spotlight_post_investigated, spotlight_post_created_on, spotlight_post_title FROM gv_spotlight_post WHERE (spotlight_post_block = 0 AND spotlight_member = 1) AND (member_id = ' . $_SESSION["member_id"] . ' AND spotlight_post_investigated = ' . mysql_real_escape_string($post_activity, $db) . ') ORDER BY spotlight_post_created_on DESC ';
	}
	else {
			$query = 'SELECT spotlight_post_id, spotlight_post_investigated, spotlight_post_created_on, spotlight_post_title FROM gv_spotlight_post WHERE (spotlight_post_block = 0 AND spotlight_member = 1) AND member_id = ' . $_SESSION["member_id"] . ' ORDER BY spotlight_post_created_on DESC ';
	}
	...
	$extra_title = ucfirst($_SESSION["username"]) . " posts | Genieverse Spotlight - " . $spotlight_title_description;
...
?>
              ...
              <div class="col-md-3">
                ...
                <div class="list-group">
					...
					<a href="../search.php?category=head_of_state" class="list-group-item">Head of State</a><?php
...
?>				<tr class="<?php echo $the_css; ?>caption-full">
					<td><a href="my_post.php?id=<?php echo $row["spotlight_post_id"]; ?>"><img class="img-responsive" height="60" width="60" src="../../images/service_images/spotlight/<?php echo post_image($row["spotlight_post_id"]); ?>" alt="Genieverse Spotlight - <?php echo $spotlight_title_description?>"></a></td>
					<td><a href="my_post.php?id=<?php echo $row["spotlight_post_id"]; ?>"><?php echo truncate_text($row["spotlight_post_title"], 80, "No title"); ?></a></td>
					<td><?php echo custom_date($row["spotlight_post_created_on"], "F t, Y. g:ia"); ?></td>
					<td><a class="btn btn-default" href="my_posts.php?delete=<?php echo $row["spotlight_post_id"]; ?>">Delete</a></td>
				</tr>
<?php
	}//End of while loop
