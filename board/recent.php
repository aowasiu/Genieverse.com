<?php
	...
	$query = 'SELECT board_comment_id, board_post_id, board_comment_edited_on, board_comment FROM gv_board_post_comments WHERE (board_comment_delete = 0 AND board_comment_block = 0) ORDER BY board_post_id DESC ';
	...
	$extra_title = "Recent posts | Genieverse Board - " . $board_meta_description;	
...
?>
		...
		echo truncate_text(modified_for_url($row["board_comment"], "&Post="), 40, "No detail."); ?>"><?php echo html_entity_decode($row["board_comment"]); ?></a></h4>
            </div>
<?php
		}
	}
			...
			?>
            <div class="col-md-4 portfolio-item">
                <p>Sorry. There're no recent Board posts.</p>
			</div><?php
	}?>
        </div>
		<!-- /.row -->
