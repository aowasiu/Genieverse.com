<?php
	...
	$query = 'SELECT board_post_id, board_category_id, cc_id, board_post_created_on, board_post_title FROM gv_board_post WHERE (board_member = 1 AND board_post_block = 0) AND (member_id = ' . $get_id . ')ORDER BY board_post_id DESC ';
	...
	$extra_title = member_username($get_id) . "'s posts | Genieverse Board - " . $board_meta_description;	
...
<?php require_once('../Server_Includes/scripts/board_scripts/board_outer_page_header.php'); ?>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><small><?php echo member_username($get_id); ?>'s Board posts</small></h1>
            </div>
        </div>
        <!-- /.row -->
<?php
