<?php
	...
	/*
		This script searches Board table of posts to retrieve posts matching the 'search_keywords'.
		It also draws from Board post image table, related photos or images
	*/
	...
	if(!isset($url_category_name) || $url_category_name == "")
	{
		...
		$_SESSION["errand"] = 'You need to select a category in the <a href="categories.php?service=board">Categories</a> section to view posts.';
	...
	require_once('../Server_Includes/scripts/board_scripts/functions_for_board.php');
	...
	if($category_id == 0)
	{
		...
		$_SESSION["errand"] = 'The category you requested is not available.<br/><a href="categories.php?service=board">Click here to choose from available categories</a>';
		header("Location: ../issues.php");
		exit;
	}

	$query = 'SELECT board_post_id, cc_id, board_category_id, board_post_title, board_post_body, board_post_created_on FROM gv_board_post WHERE (board_member = 1 AND board_post_block = 0) AND (board_category_id = ' . $category_id . ') ORDER BY board_post_id DESC ';
/*
	(board_post_investigated = 1 AND board_category_id = ' . $category_id . ') ORDER BY board_post_id DESC ';
*/
	...
	if($rows == 0)
	{
		...
		$_SESSION["errand"] = "There're no posts, yet, for this category<br/><a href='categories.php?service=board'>Click here to choose another category besides " . ucfirst($category_name) . '</a>';
		header("Location: ../issues.php");
		exit;
	}
	...
	$extra_title = "Search | Genieverse Board - " . $board_meta_description;
...
?>
		 ...
		 echo modified_for_url($row["board_post_id"]), "&Category=");
		echo truncate_text(modified_for_url($row["board_post_title"], "&Post="), 40, "No detail."); ?>"><?php echo $row["board_post_title"]; ?></a></h3>
            </div>
<?php
       }
	}
