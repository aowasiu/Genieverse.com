<?php
	...
	$query = 'SELECT board_post_id, board_category_id, cc_id, board_post_created_on, board_post_title FROM gv_board_post WHERE (board_member = 1 AND board_post_block = 0) AND (board_post_hot = 1) ORDER BY board_post_id DESC ';
	...
	$special = "set";
	$extra_title = "Hot Topics | Genieverse Board - " . $board_meta_description;
...
<?php require_once('../Server_Includes/scripts/board_scripts/board_outer_page_header.php'); ?>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><small>Hot topics</small></h1>
            </div>
        </div>
        <!-- /.row -->
...
?><div class="col-md-4 portfolio-item">
		...
		echo truncate_text(modified_for_url($row["board_post_title"], "&Post="), 40, "No detail."); ?>"><?php echo html_entity_decode($row["board_post_title"]); ?></a></h4>
			...
			?>
            <div class="col-md-4 portfolio-item">
                <p>Sorry. There're no Hot topics yet.</p>
			</div><?php
	}?>
        </div>
		<!-- /.row -->
