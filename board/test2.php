...
/*		echo '
                    <div class="row">
                        <div class="col-md-12">
                            <p><b>On</b> ' . change_date_long($row["board_comment_created_on"]) . ', <b>' . member_username($row["member_id"]) . '</b> said:</p>
                            <p style="background: #FFFFFF;" class="alert alert-info">' . $row["board_comment"] . '</p>
							<p></p>
                            ' . '<a href="">Quote</a> |' . ' 
                            <a href="../thanks_for_reporting.php?db=boardComment&id=' . $row["board_comment_id"] . '">Report</a> | 
                            ' . '<a href="">Boo!</a> | 
                            <a href="">Hail</a> |' . '
                        </div>
                    </div>
';
			...
			echo '
                        ...
                        <h1><a href="post_view.php?id=' . $data . '">' . html_entity_decode($row["board_post_title"]) . '</a></h1>';
	}
*/
echo '
                    <div class="row">
                        <div class="col-md-12">
                            <p><b>On</b> ' . change_date_long($row["board_post_created_on"]) . ', <b>' . member_username($row["member_id"]) . '</b> said:</p>
                            <p style="background: #FFFFFF;" class="alert alert-info">' . $row["board_post_title"] . '</p>
							<div class="row">' .
							html_entity_decode($row["board_post_body"]) . '<br/>' . 
							html_entity_decode($row["board_post_source_website"]) . '<br/>
                            <a href="">Quote</a> | 
                            <a href="../thanks_for_reporting.php?db=board&id=' . $row["board_post_id"] . '">Report</a> | 
                            <a href="board_features.php?db=boo&id=' . $row["board_post_id"] . '">Boo!</a> | 
                            <a href="board_features.php?db=thumbsup&id=' . $row["board_post_id"] . '">Thumbs up</a> | 
                            <a href="board_features.php?db=thumbsdown&id=' . $row["board_post_id"] . '">Thumbs down</a> |
							</div>
                        </div>
                    </div>
